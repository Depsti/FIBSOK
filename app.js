...
const SUPABASE_URL_DEFAULT = 'https://vpgwwboohfcdtiiyobpk.supabase.co';
const SUPABASE_KEY_DEFAULT = 'sb_publishable_HI2FFwBFt8dxBxAqhas_uQ_rMIImceZ';

let supabaseClient = null;
const supabaseUrl = localStorage.getItem(SUPABASE_URL_KEY) || SUPABASE_URL_DEFAULT;
const supabaseKey = localStorage.getItem(SUPABASE_KEY_KEY) || SUPABASE_KEY_DEFAULT;
...
function handleImageSelect(file, hiddenId, previewId) {
  ...
  previewContainer.innerHTML = '<div class="upload-loading">Komprimiere & Lade hoch...</div>';
  ...
  reader.onload = function(e) {
    ...
    img.onload = function() {
      ...
      canvas.toBlob(async function(blob) {
        ...
        document.getElementById(hiddenId).value = imageUrl;
        previewContainer.innerHTML = `<img src="${imageUrl}"><button type="button" class="btn-remove" onclick="clearImage('${hiddenId}','${previewId}')">Entfernen</button>`;
      }, 'image/jpeg', 0.7);
    };
...
async function deleteCase(id){
  ...
  if(supabaseClient){
    try{
      await supabaseClient.from('entries').delete().eq('caseId',id);
    ...
    document.body.insertAdjacentHTML('beforeend', `
      <div id="lightbox-overlay" class="lightbox-overlay" onclick="closeLightbox()">
        <div class="lightbox-content" onclick="event.stopPropagation()">
          ...
          <img id="lightbox-img" src="" alt="Lightbox-Ansicht">
        </div>
      </div>
    `);
